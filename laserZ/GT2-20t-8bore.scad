...
motor_shaft = 8.4;	// NEMA17 motor shaft exact diameter = 5
...
pulley_t_ht = 6;	// length of toothed part of pulley, standard = 1
...
keyway = 0;         // Keyway size, 0 for none
...
union()
{
  ...
  intersection()
  {
    translate([(-motor_shaft+.2)/2, motor_shaft/3,0])
      cube([motor_shaft+.2, motor_shaft/2, pulley_t_ht+idler_ht*2]);
    cylinder(d=motor_shaft+.2, h=pulley_t_ht+idler_ht*2+.2);
  }
  difference()
  {
    cylinder(d=motor_shaft+2, h=pulley_t_ht+idler_ht*2+0.5,$fn=motor_shaft*4);
    translate([0,0,-0.1])
      cylinder(d=motor_shaft, h=pulley_t_ht+idler_ht*2+0.7,$fn=motor_shaft*4);
  }
}
// Functions
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
        ...
        if (keyway > 0) {
          translate([-keyway/2,(motor_shaft-keyway)/2,-1]) cube([keyway,keyway,pulley_b_ht + pulley_t_ht + retainer_ht + 2]);
        } 
